...
?>
	  ...
	  <?php       
	 	// busca total de compras pagas   ;

		@$query_contas_pagas_tot = $pdo->query("SELECT SUM(valor_total) as tot_val from vender_servico where cliente_nome = '$txt_cli' AND status = 'Pago'");
		@$res_contas_pagas_tot = $query_contas_pagas_tot->fetchAll(PDO::FETCH_ASSOC);
		@$cliente_contas_pagas_tot = $res_contas_pagas_tot[0]['tot_val'];


		@$query_contas_canceladas_tot = $pdo->query("SELECT SUM(valor_total) as tot_val2 from vender_servico where cliente_nome = '$txt_cli' AND status = 'Cancelado'");
		@$res_contas_canceladas_tot = $query_contas_canceladas_tot->fetchAll(PDO::FETCH_ASSOC);
		@$cliente_contas_canceladas_tot = $res_contas_canceladas_tot[0]['tot_val2'];


		@$query_contas_espera_tot = $pdo->query("SELECT SUM(valor_total) as tot_val3 from vender_servico where cliente_nome = '$txt_cli' AND status != 'Cancelado' AND status != 'Pago'");
		@$res_contas_espera_tot = $query_contas_espera_tot->fetchAll(PDO::FETCH_ASSOC);
		@$cliente_contas_espera_tot = $res_contas_espera_tot[0]['tot_val3'];




		// Calculo das sessões pagas e pendentes...
		@$query_sessao_tot = $pdo->query("SELECT SUM(qtd) as tot_val4 from vender_servico where cliente_nome = '$txt_cli' AND status = 'Pago'");
		@$res_sessao_tot = $query_sessao_tot->fetchAll(PDO::FETCH_ASSOC);
		@$cliente_sessao_tot_pago = (int)@$res_sessao_tot[0]['tot_val4'];

		@$query_sessao_tot_usadas = $pdo->query("SELECT SUM(qtd_usados) as tot_val5 from vender_servico where cliente_nome = '$txt_cli' AND status = 'Pago'");
		@$res_sessao_tot_usadas = $query_sessao_tot_usadas->fetchAll(PDO::FETCH_ASSOC);
		@$cliente_sessao_tot_usadas = (int)@$res_sessao_tot_usadas[0]['tot_val5'];

		// Conta as sessoes que ainda estão pagas e nao realisadas
		@$cliente_sessao_tot_sum = (int)@$cliente_sessao_tot_pago - (int)@$cliente_sessao_tot_usadas;
	...
	<div class="row"> <!-- inicio grupo  Mostra os totais do cliente -->
		...
		<div class="col-md-3">
			<div class="mb-4">	
				 Tot. de Compras Pagas: 
				 	R$ <span style="color: royalblue;"><?php echo number_format($cliente_contas_pagas_tot, 2, ',', '.'); ?></span>	
			</div>		 
		</div>

		<div class="col-md-3">
			<div class="mb-4">
				 Tot. Sessões Ativas: <span style="color: seagreen;">0<?php echo $cliente_sessao_tot_pago ?> </span>Uni.
			</div>
		</div>

		<div class="col-md-3">
			<div class="mb-4">
				 Tot. Pagamento em Espera:  
          R$ <span style="color: indianred;"><?php echo number_format($cliente_contas_espera_tot, 2, ',', '.'); ?></span>	
       </div>
		</div>

		<div class="col-md-3">
			<div class="mb-4">
				 Tot. Compras Canceladas:  
        	R$ <span style="color: indianred;"><?php echo number_format($cliente_contas_canceladas_tot, 2, ',', '.'); ?></span>	 
       </div>
		</div>
...
<div class="mt-4" style="margin-right:25px">
	...
	$query = $pdo->query("SELECT * from vender_servico where cliente_nome = '$txt_cli' order by created desc");
