...
if($id == ""){
	...
	if($imagem != 'sem-foto.jpg'){
		$res = $pdo->prepare("UPDATE orcamentos SET  id_form_pag = :id_form_pag, obs = :obs, usuario = '$id_usuario', valor_total = :valor_total, status = :status, produto = :produto, foto = :foto, modificado = '$created', ultima_modificacao = '$id_usuario', qtd = '$qtd', situacao = '$situacao' WHERE cliente_nome = :id");
		$res->bindValue(":foto", $imagem);
	}else{
		$res = $pdo->prepare("UPDATE orcamentos SET  id_form_pag = :id_form_pag, obs = :obs, usuario = '$id_usuario', valor_total = :valor_total, status = :status, produto = :produto, foto = :foto, modificado = '$created', ultima_modificacao = '$id_usuario', qtd = '$qtd', situacao = '$situacao' WHERE cliente_nome = :id");
	}
