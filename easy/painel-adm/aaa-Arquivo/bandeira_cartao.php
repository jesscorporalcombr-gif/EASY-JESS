...
<h4 style="font-size: 14px;  font-style:normal;" class="text-uppercase">Bandeiras Cartões Débito</h4>
...
<div class="col-md-9">  
	<div class="col-md-3">
		...
		<a style="font-size: 12px;  " href="index.php?pagina=<?php echo $pag ?>&funcao=novo"
		 type="button" class="btn btn-outline-primary">Nova Bandeira Débito</a>
	</div>
</div>  

<div class="mt-4" style="margin-right:25px">
	<?php 
	$query = $pdo->query("SELECT * from bandeira_cartao order by id desc");
			...
			<table id="example" class="table table-hover my-4" style="width:100%">
				...
				<tbody>
					...
					for($i=0; $i < $total_reg; $i++){
						...
						<tr>
							<td>  <?php echo $res[$i]['bandeira'] ?> </td>
							<td> <?php echo $res[$i]['conta_bancaria'] ?>  </td>
							<td>  <?php echo $res[$i]['taxa_cartao_de_debito'] ?> </td>
							<td>  <?php echo $res[$i]['prazo'] ?>  </td>
...
if(@$_GET['funcao'] == "editar"){
	$titulo_modal = 'Editar Bandeira';
	$query = $pdo->query("SELECT * from bandeira_cartao where id = '$_GET[id]'");
	$res = $query->fetchAll(PDO::FETCH_ASSOC);
	$total_reg = @count($res);
	if($total_reg > 0){ 
		...
		$taxa_cartao_de_debito = $res[0]['taxa_cartao_de_debito'];	
		$prazo = $res[0]['prazo'];
		$foto = $res[0]['foto'];
		$id = $res[0]['id'];
	...
	$titulo_modal = 'Adicionar Bandeira Débito';
}
?>
			...
			<form method="POST" id="form">
				<div class="modal-body">
					...
					<div class="row"> <!-- inicio do grupo  s2 -->
						<div class="col-md-8">
							<div class="mb-5">
								<label for="exampleFormControlInput1" class="form-label">Taxa Cartao de debito</label>
								<input type="text" class="form-control" id="taxa_cartao_de_debito" name="taxa_cartao_de_debito" placeholder="Taxa Cartao de debito"  value="<?php echo @$taxa_cartao_de_debito ?>">
							</div> 
						</div>
...
<script type="text/javascript">
	function mostrarDados(nome, foto){
		...
		$('#imagem-registro').attr('src', '../img/bandeira_cartao/' + foto);
