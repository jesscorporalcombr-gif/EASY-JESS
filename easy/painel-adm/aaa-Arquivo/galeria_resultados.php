...
<h4 style="font-size: 14px;  font-style:normal;" class="text-uppercase">Galeria/Resultados</h4>
...
<div class="mt-4" style="margin-right:25px">
	<?php 
	$query = $pdo->query("SELECT * from galeria_resultados order by id desc");
...
if(@$_GET['funcao'] == "editar"){
	...
	$query = $pdo->query("SELECT * from galeria_resultados where id = '$_GET[id]'");
	$res = $query->fetchAll(PDO::FETCH_ASSOC);
	$total_reg = @count($res);
	if($total_reg > 0){ 
		@$valor = $res[0]['valor'];
...
<script type="text/javascript">
	...
	function carregarImg() {
        ...
        if(resultado[1] === 'pdf'){
        	$('#target').attr('src', "../img/galeria_resultados/pdf.png");
        	return;
        }
