...
<h4 style="font-size: 14px;  font-style:normal;" class="text-uppercase">Contas Contábeis</h4>
...
<div class="col-md-6"> <!--  Botão com edição para roxo e rosa -->
	<div class="col-md-4">
		 ...
		 <a style="font-size: 12px;  " href="index.php?pagina=<?php echo $pag ?>&funcao=novo" type="button" class="btn btn-outline-primary">Nova Conta</a>
	</div>
</div>
...
<div class="mt-4" style="margin-right:25px">
	<?php 
	$query = $pdo->query("SELECT * from contas_contabeis order by id desc");
				...
				<tbody>
					...
					for($i=0; $i < $total_reg; $i++){
						...
						<tr>
							...
							<td><a href="../img/<?php echo $pag ?>/<?php echo $arquivo_pasta ?>" title="Ver Arquivo" style="text-decoration: none" target="_blank">
...
if(@$_GET['funcao'] == "editar"){
	...
	$query = $pdo->query("SELECT * from contas_contabeis where id = '$_GET[id]'");
	$res = $query->fetchAll(PDO::FETCH_ASSOC);
	$total_reg = @count($res);
	if($total_reg > 0){ 
		$valor = $res[0]['valor'];
...
<script type="text/javascript">
	...
	function carregarImg() {
        ...
        if(resultado[1] === 'pdf'){
        	$('#target').attr('src', "../img/contas_contabeis/pdf.png");
        	return;
        }
