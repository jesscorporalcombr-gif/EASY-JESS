...
<h4 style="font-size: 14px;  font-style:normal;" class="text-uppercase">Equipamento</h4>
...
<div class="col-md-7"> <!--  Botão com edição para roxo e rosa -->
	<div class="col-md-3">
		...
		<a style="font-size: 12px;  " href="index.php?pagina=<?php echo $pag ?>&funcao=novo"
		 type="button" class="btn btn-outline-primary">Novo Equipamento</a>
	</div>
</div>
...
<div class="mt-4" style="margin-right:25px">
	<?php 
	$query = $pdo->query("SELECT * from equipamento order by id desc");
				...
				<tbody>
					...
					for($i=0; $i < $total_reg; $i++){
						...
						?>
							...
							<td> <h6 style="font-size: 12px;  "> <?php echo $res[$i]['descricao'] ?> </h6> </td>

							<td> <h6 style="font-size: 12px;  "> <?php echo $res[$i]['patrimonio'] ?> </h6> </td>
...
if(@$_GET['funcao'] == "editar"){
	...
	$query = $pdo->query("SELECT * from equipamento where id = '$_GET[id]'");
	$res = $query->fetchAll(PDO::FETCH_ASSOC);
	$total_reg = @count($res);
	if($total_reg > 0){ 
		$nome = $res[0]['nome'];
		...
		$patrimonio = $res[0]['patrimonio'];
		
		$foto_edit = $res[0]['foto'];

		

		$extensao2 = strchr($foto_edit, '.'); //busca a palavra apos o ponto
	...
	$titulo_modal = 'Inserir Equipamento';
}
?>
					...
					<div class="row">
						...
						<div class="col-md-6">
							<div class="mb-5">
								...
								<label for="exampleFormControlInput1" class="form-label">É Patrimonio</label>
								

							<select class="form-select mt-1" aria-label="Default select example" name="patrimonio">
								
								<option <?php if(@$patrimonio == 'Não'){ ?> selected <?php } ?>  value="Não">Não</option>

								<option <?php if(@$patrimonio == 'Sim'){ ?> selected <?php } ?>  value="Sim">Sim</option>
					...
					<small><div align="center" class="mt-1" id="mensagem">
					    ...
					    <div class="col-md-4">
							<div class="form-group">
								<label >Foto</label>
								<input type="file" value="<?php echo @$arquivo_extensao2 ?>"  class="form-control-file" id="imagem" name="imagem" onChange="carregarImg();">
							</div>							
						</div>

						<div class="col-md-4">
							<div id="divImgConta" class="mt-4">
								<?php if(@$arquivo_extensao2 != ""){ ?>
...
<script type="text/javascript">
	function mostrarDados(nome, foto){
		...
		$('#imagem-registro').attr('src', '../img/equipamentos/' + foto);
