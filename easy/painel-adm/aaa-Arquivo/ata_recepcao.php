...
<h4 style="font-size: 14px;  font-style:normal;" class="text-uppercase">Ata Diaria Recepção</h4>

<div class="col-md-7"> <!--  Botão  -->
	<div class="col-md-4">
		</br>

		 <a style="font-size: 12px;  " href="index.php?pagina=<?php echo $pag ?>&funcao=novo" type="button" class="btn btn-outline-primary">Nova Ata</a>
	</div>
</div>


<!--<a href="index.php?pagina=<?php echo $pag ?>&funcao=novo" type="button" class="btn btn-secondary mt-2">Nova ata_recepcao</a> -->

<div class="mt-4" style="margin-right:25px">
	<?php 
	$query = $pdo->query("SELECT * from ata_recepcao order by id desc");
...
if(@$_GET['funcao'] == "editar"){
	$titulo_modal = 'Editar Ata';
	$query = $pdo->query("SELECT * from ata_recepcao where id = '$_GET[id]'");
	$res = $query->fetchAll(PDO::FETCH_ASSOC);
	$total_reg = @count($res);
	if($total_reg > 0){ 
		$valor = $res[0]['valor'];
		...
		<div class="modal-content">
			...
			<form method="POST" id="form">
				<div class="modal-body">
					...
					<div class="mb-3">
						<label for="exampleFormControlInput1" class="form-label">Descrição da Ata</label>						
						<textarea type="text" class="form-control" id="descricao" name="descricao"  placeholder="Descrição da Ata" maxlength="200"><?php echo @$descricao ?></textarea>
					</div> 
...
<div class="modal fade" tabindex="-1" id="modalDeletar" >
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title">Excluir Ata</h5>
				<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
			</div>
		...
		<div class="modal-content">
			...
			<form method="POST" id="form-baixar">
				<div class="modal-body">
					...
					<p>Deseja Realmente confirmar o Recebimento do pagamento desta Ata?</p>
...
<script type="text/javascript">
	...
	function carregarImg() {
        ...
        if(resultado[1] === 'pdf'){
        	$('#target').attr('src', "../img/ata_recepcao/pdf.png");
        	return;
        }
