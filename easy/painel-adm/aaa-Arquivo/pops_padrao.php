...
<h4 style="font-size: 14px;  font-style:normal;" class="text-uppercase">Gravar POP (procedimento operacional padrão)</h4>
...
<div class="col-md-7"> <!--  Botão  -->
	<div class="col-md-4">
		...
		<a style="font-size: 12px;  " href="index.php?pagina=<?php echo $pag ?>&funcao=novo"
		 type="button" class="btn btn-outline-primary">Novo POP</a>
	</div>
</div>
...
<div class="mt-4" style="margin-right:25px">
	<?php 
	$query = $pdo->query("SELECT * from pops_padrao order by id desc");
...
if(@$_GET['funcao'] == "editar"){
	$titulo_modal = 'Editar POP';
	$query = $pdo->query("SELECT * from pops_padrao where id = '$_GET[id]'");
	$res = $query->fetchAll(PDO::FETCH_ASSOC);
	$total_reg = @count($res);
	if($total_reg > 0){ 
		$nome = $res[0]['nome'];
			...
			<form method="POST" id="form">
				<div class="modal-body">
					...
					<div class="row">
						...
						<div class="col-md-12">
							<div class="mb-5">
								<label for="exampleFormControlInput1" class="form-label">Titulo POP</label>
...
<script type="text/javascript">
	function mostrarDados(nome, foto){
		...
		$('#imagem-registro').attr('src', '../img/pops_padrao/' + foto);
