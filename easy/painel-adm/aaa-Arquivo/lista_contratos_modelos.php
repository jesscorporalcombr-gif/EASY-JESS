...
<h4 style="font-size: 14px;  font-style:normal;" class="text-uppercase">Modelo de Documento</h4>
...
<div class="col-md-7"> <!--  Botão -->
	<div class="col-md-4">
		...
		<a style="font-size: 12px;  " href="index.php?pagina=<?php echo $pag ?>&funcao=novo"
		 type="button" class="btn btn-outline-primary">Novo Modelo</a>
	</div>
</div>
...
<div class="mt-4" style="margin-right:25px">
	<?php 
	$query = $pdo->query("SELECT * from lista_contratos_modelos order by id desc");
		...
		<small>
			<table id="example" class="table table-hover my-4" style="width:100%">
				...
				<tbody>
					...
					<?php 
					//esse for é que faz apereccer a miniatura da imagem
					...
					for($i=0; $i < $total_reg; $i++){
						...
						elseif ($extensao == '.doc') {
							...
							<td><?php echo implode('/', explode('-', $res[$i]['created'])); ?></td>
...
if(@$_GET['funcao'] == "editar"){
	$titulo_modal = 'Editar Modelo';
	$query = $pdo->query("SELECT * from lista_contratos_modelos where id = '$_GET[id]'");
	$res = $query->fetchAll(PDO::FETCH_ASSOC);
	$total_reg = @count($res);
	if($total_reg > 0){ 
		$nome = $res[0]['nome'];
					...
					<div class="row">
						...
						<div class="col-md-12">
							<div class="mb-5">
								...
								<input type="text" class="form-control" id="nome" name="nome" placeholder="Modelo" required="" value="<?php echo @$nome ?>">
							</div> 
						</div>
...
<script type="text/javascript">
	function mostrarDados(nome, foto){
		...
		$('#imagem-registro').attr('src', '../img/lista_contratos_modelos/' + foto);
