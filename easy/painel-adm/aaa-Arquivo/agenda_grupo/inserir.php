...
if($antigo2 != $email){
	$query_con = $pdo->prepare("SELECT * from agenda_grupo WHERE email = :email");
...
// EVITAR DUPLICIDADE NO CPF
	$query_con = $pdo->prepare("SELECT * from agenda_grupo WHERE cpf = :cpf");
...
$caminho = '../../img/agenda_grupo/' .$nome_img;
...
if($id == ""){
	$res = $pdo->prepare("INSERT INTO agenda_grupo SET nome = :nome, email = :email, cpf = :cpf, senha = :senha, nivel = :nivel, foto = :foto");
	...
	if($imagem != 'sem-foto.jpg'){
		$res = $pdo->prepare("UPDATE agenda_grupo SET nome = :nome, email = :email, cpf = :cpf, foto = :foto, senha = :senha, nivel = :nivel WHERE id = :id");
		$res->bindValue(":foto", $imagem);
		
		}else{
			$res = $pdo->prepare("UPDATE agenda_grupo SET nome = :nome, email = :email, cpf = :cpf, senha = :senha, nivel = :nivel WHERE id = :id");
