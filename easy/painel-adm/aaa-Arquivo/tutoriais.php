...
<h4 style="font-size: 14px;  font-style:normal;" class="text-uppercase">Tutoriais</h4>


<div class="col-md-7"> <!--  Botão  -->
	<div class="col-md-4">
		</br>
		<a style="font-size: 13px; href="index.php?pagina=<?php echo $pag ?>&funcao=novo" type="button" class="btn btn-outline-primary">Novo Tutorial</a>
	</div>
</div>
...
<div class="mt-4" style="margin-right:25px">
	<?php 
	$query = $pdo->query("SELECT * from tutoriais order by id DESC");
	$res = $query->fetchAll(PDO::FETCH_ASSOC);
	$total_reg = @count($res);
	if($total_reg > 0){ 
		?>
				...
				<tbody>
					...
					for($i=0; $i < $total_reg; $i++){
						...
						<tr>
							...
							<td>
								<a href="<?php echo $res[$i]['interesse'] ?>" title="Ver Url" style="text-decoration: none" target="_blank">

								<?php echo $res[$i]['interesse'] ?>
							</td>
...
if(@$_GET['funcao'] == "editar"){
	...
	$query = $pdo->query("SELECT * from tutoriais where id = '$_GET[id]'");
	$res = $query->fetchAll(PDO::FETCH_ASSOC);
	$total_reg = @count($res);
	if($total_reg > 0){ 
				...
				<div class="modal-body">
					...
					<div class="col-md-10">
							<div class="mb-5">
								...
								<textarea type="text" class="form-control" id="interesse" name="interesse"  placeholder="Url/link" required="" maxlength="400"><?php echo @$interesse ?></textarea>
