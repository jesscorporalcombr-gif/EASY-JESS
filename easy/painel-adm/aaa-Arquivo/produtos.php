...
<h4 style="font-size: 14px;  font-style:normal;" class="text-uppercase">Produtos</h4>
...
<div class="col-md-7"> <!--  Botão  -->
	<div class="col-md-4">
		 ...
		 <a style="font-size: 12px;  " href="index.php?pagina=<?php echo $pag ?>&funcao=novo" type="button" class="btn btn-outline-primary">Novo Produto</a>
				...
				<tbody>
					...
					for($i=0; $i < $total_reg; $i++){
						...
						<tr>
							...
							<!--<td><?php echo $res[$i]['em_promocao'] ?></td>-->
							...
							<td>
								...
								'<?php echo $res[$i]['fornecedor'] ?>',
								'<?php echo $res[$i]['local'] ?>',
...
if(@$_GET['funcao'] == "editar"){
	...
	if($total_reg > 0){ 
		...
		$local = $res[0]['local'];
		$foto_edit = $res[0]['foto'];
		$estoque_min = $res[0]['estoque_min'];
		$ativo = $res[0]['ativo'];


		$id_fornecedor_modal = $fornecedor;
		$query_fornecedor_modal = $pdo->query("SELECT * from  fornecedores where id = '$id_fornecedor_modal' limit 1");
		$res_fornecedor_modal = $query_fornecedor_modal->fetchAll(PDO::FETCH_ASSOC);
		@$nome_fornecedor_modal = $res_fornecedor_modal[0]['nome'];
					...
					<div class="row"> <!-- grupo -->
						<div class="col-md-4">
								<div id="divImgConta" class="mt-4">
									<?php if(@$foto_edit != ""){ ?>
										<img src="../img/<?php echo $pag ?>/<?php echo $foto_edit ?>"  width="150px" id="target">
									<?php  }else{ ?>
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body mb-4">
				...
				<b>Valor Compra: </b>
				<span id="valor-compra"></span>
				<hr>

				<b>Estoque: </b>
				<span id="estoque_"></span>
				<hr>

				<b>Estoque Minimo: </b>
				<span id="estoque-min_"></span>
				<hr>

				<!--<b>Fornecedor: </b>
				<span id="fornecedor_"></span>
				<hr>-->
				...
				<div id="div-forn">
					<span class="mr-4">
						<b>Fornecedor: </b> <!-- tratar para aparecer o nome -->
...
<script type="text/javascript">
	function mostrarDados(nome, codigo, valor_venda, valor_compra, estoque, estoque_min, descricao, fornecedor, local, lucro, foto, categoria, nome_forn, tel_forn){
		event.preventDefault();
		...
		$('#codigo_').text(codigo);
		$('#valor-venda').text(valor_venda);
		$('#valor-compra').text(valor_compra);
		$('#estoque_').text(estoque);
		$('#estoque-min_').text(estoque_min);
		$('#categoria-registro').text(categoria);
		$('#descricao-registro').text(descricao);
		$('#fornecedor_').text(fornecedor);
		$('#local_').text(local);
		$('#lucro_').text(lucro);
