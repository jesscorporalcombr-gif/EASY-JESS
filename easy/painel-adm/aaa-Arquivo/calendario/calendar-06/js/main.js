var Calendar = function(t) {
    this.divId = t.RenderID ? t.RenderID : '[data-render="calendar"]', this.DaysOfWeek = t.DaysOfWeek ? t.DaysOfWeek : ["Dom", "Seg", "Ter", "Qua", "Qui", "Sex", "Sab"], this.Months = t.Months ? t.Months : ["Janeiro", "Fevereiro", "Março", "Abril", "Maio", "Junho", "Julho", "Agosto", "Setembro", "Outubro", "Novembro", "Dezembro"];
    var e = new Date;
    ...
    for (var h, l = dm = "M" == this.f ? 1 : 0 == o ? -5 : 2, v = (c = 0, 0); v < 6; v++) {
        ...
        for (var m = 0; m < 7; m++) {
            if ((h = c + dm - o) < 1) d += '<div class="cell disable">' + (u - o + l++) + "</div>";
                ...
                d += '<div class="cell' + (r == h && this.CurrentMonth == n && this.CurrentYear == a ? " active" : "") + '"onclick="selectDate(this, ' + h + ', ' + e + ', ' + t + ');"><span>' + h + '</span></div>';
            }
            c % 7 == 6 && h >= i && (v = 10), c++
        }
        d += "</div>"
...
function selectDate(clickedCell, day, month, year) {
    
    var selectedDate = new Date(year, month, day);
    
    var formattedDate = year + '-' + ('0' + (month + 1)).slice(-2) + '-' + ('0' + day).slice(-2);
    
    window.dataCalend = formattedDate; // Atribui a data a uma variável global
    ...
    // Adicione a classe 'selected' à célula clicada
    clickedCell.classList.add('selected');
    document.querySelector('.cell.selected').addEventListener('click', function() {
        this.blur();
    });
}