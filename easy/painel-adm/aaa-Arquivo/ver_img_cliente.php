...
<nav class="navbar navbar-expand-lg navbar-light bg-light">
    ...
    <h4 style="font-size: 14px;  font-style:normal;" class="text-uppercase">Ver Imagens Clientes</h4>
	&nbsp;&nbsp;&nbsp;&nbsp;
	 ...
	 <!--- /////////////// inicio busca por cliente /////////////// -->   
	  <div  id="conteudoNavbarSuportado">
	      <ul class="navbar-nav mr-auto"></ul>
		      <!--- /////////////// busca por cliente/////////////// -->
		      <form class="btn-group" method="POST" action="index.php?pagina=ver_img_cliente"> <!--Classe p/ a busca-->    
			        <select style=" width: 170px; height: 25px; font-size: 12px; " class="mt-2" data-width="100%"  id="selec_cli" name="txt_cli"> <!--- Variavel p/ a busca -->         
			            <?php
			              $query = "SELECT DISTINCT usuario_referente_id FROM img_cliente ORDER BY usuario_referente_id asc";
...
<div class="col-md-7">  
	<div class="col-md-4">
		 ...
		 <a style="font-size: 12px;  " href="index.php?pagina=<?php echo $pag ?>&funcao=novo" type="button" class="btn btn-outline-primary">Nova Imagem</a>
	</div>
</div> 

<div class="mt-4" style="margin-right:25px">
	<?php 
	$query = $pdo->query("SELECT * from img_cliente where usuario_referente_id = '$txt_cli' order by id desc");
				...
				<tbody>
					...
					for($i=0; $i < $total_reg; $i++){
						...
						<tr style="padding: 9px;" >
							...
							<td  style=" width: 260px; height: 200px; padding: 7px; border-radius: 5px;

						    box-shadow: -2px -2px px #bcbcbc, 2px 2px 2px #bcbcbc;
							-webkit-box-shadow: -2px -2px 2px #bcbcbc, 2px 2px 2px #bcbcbc;
							-moz-box-shadow: -2px -2px 2px #bcbcbc, 2px 2px 2px #bcbcbc;> ">

								<a href="../img/img_cliente/<?php echo $res[$i]['arquivo'] ?>" title="Ver Arquivo" style="text-decoration: none" target="_blank">
									<img src="../img/img_cliente/<?php echo $arquivo_pasta ?>" width="180">
					...
					<th style=" width: 5px; padding: 7px;" ></th>	
...
if(@$_GET['funcao'] == "editar"){
	$titulo_modal = 'Editar Imagem do Cliente';
	$query = $pdo->query("SELECT * from img_cliente where id = '$_GET[id]'");
	$res = $query->fetchAll(PDO::FETCH_ASSOC);
	$total_reg = @count($res);
	if($total_reg > 0){ 
		$valor = $res[0]['valor'];
			...
			<form method="POST" id="form">
				<div class="modal-body">
					...
					<div id="divImgConta" class="mt-4">
						<?php if(@$arquivo != ""){ ?>
							<img src="../img/img_cliente/<?php echo @$arquivo_pasta2 ?>"  width="200px" id="target">
						<?php  }else{ ?>
							<img src="../img/img_cliente/sem-foto.jpg" width="200px" id="target">
						<?php } ?>
					</div>
...
<script type="text/javascript">
	...
	function carregarImg() {
        ...
        if(resultado[1] === 'pdf'){
        	$('#target').attr('src', "../img/img_cliente/pdf.png");
        	return;
        }
