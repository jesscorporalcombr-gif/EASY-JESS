...
?>
	...
	<h4 style="font-size: 14px;  font-style:normal;" class="text-uppercase">Carta de Clientes</h4>
...
<div class="col-md-7"> <!--  Botão -->
	<div class="col-md-4">
		 ...
		 <a style="font-size: 12px;  " href="index.php?pagina=<?php echo $pag ?>&funcao=novo" type="button" class="btn btn-outline-primary">Nova Conexão</a>
	</div>
</div>
...
<div class="mt-4" style="margin-right:25px">
	<?php 
	$query = $pdo->query("SELECT * from carta_de_clientes order by id desc");
			...
			<table id="example" class="table table-hover my-4" style="width:100%">
				...
				<tbody>
					...
					for($i=0; $i < $total_reg; $i++){
						...
						$id_usu = $res[$i]['usuario_referente_id'];
						$query_p = $pdo->query("SELECT * from usuarios where id = '$id_usu'");
						$res_p = $query_p->fetchAll(PDO::FETCH_ASSOC);
						@$nome_usu = $res_p[0]['nome'];

						// criado por
						$id_usu1 = $res[$i]['usuario'];
						$query_p1 = $pdo->query("SELECT * from usuarios where id = '$id_usu1'");
						$res_p1 = $query_p1->fetchAll(PDO::FETCH_ASSOC);
						$nome_usu1 = $res_p1[0]['nome'];

						// busca o nome do usuario referente desse documento
						@$id_usu2 = $res[$i]['cliente_referente_id'];
...
if(@$_GET['funcao'] == "editar"){
	...
	$query = $pdo->query("SELECT * from carta_de_clientes where id = '$_GET[id]'");
	$res = $query->fetchAll(PDO::FETCH_ASSOC);
	$total_reg = @count($res);
	if($total_reg > 0){ 
		...
		@$cliente_referente_id = $res[0]['cliente_referente_id'];
		...
		// busca o nome do usuario atraves do id
		$id_cliente_modal = $cliente_referente_id;
		$query_cliente_modal = $pdo->query("SELECT * from clientes where id = '$id_cliente_modal' limit 1");
		$res_cliente_modal = $query_cliente_modal->fetchAll(PDO::FETCH_ASSOC);
		@$nome_cliente_modal = $res_cliente_modal[0]['nome'];
	...
	$titulo_modal = 'Inserir Nova Carta';
}
?>
				...
				<div class="modal-body">
					...
					<div class="col-md-12">
						<div class="mb-3"><!-- Arrumar o salvamento em tabela-->
							<label for="exampleFormControlInput1" class="form-label">Profissional Referente</label>
				              ...
				              <?php
				               ...
				               ?>
				                <select data-width="100%" class="form-control mr-1" id="usuario_referente_id" required=""  
				                    ...
				                    value="<?php echo @$usuario_referente_id /*a variavel que voltará a ser salva*/ ?>" ><?php echo @$nome_profissional_modal?></option>
					...
					<div class="col-md-12">
						<div class="mb-3"><!-- Arrumar o salvamento em tabela-->
							...
							<!-- Busca do banco de dados para uma  select. Escolha dinamica -->
				              <?php
				                 $sql_code_cliente = "SELECT * FROM clientes ORDER BY nome ASC";
				                 $sql_query_cliente = $conexao->query($sql_code_cliente) or die($conexao->error);                  
				               ?>
				                <select data-width="100%" class="form-control mr-1" id="cliente_referente_id" required=""  
				                
				                    <?php if(isset($_GET['nome'])) echo "disabled"; ?>  name="cliente_referente_id" >
				                    
				                    <option class="form-control" 
				                    value="<?php echo @$cliente_referente_id /*a variavel que voltará a ser salva*/ ?>" ><?php echo @$nome_cliente_modal?></option>

				                    <?php while($nome = $sql_query_cliente->fetch_assoc()) { ?> 
...
<script type="text/javascript">
	...
	function carregarImg() {
        ...
        if(resultado[1] === 'pdf'){
        	$('#target').attr('src', "../img/carta_de_clientes/pdf.png");
        	return;
        }
