<?php 
$pag = 'termos_contratos_assinaturas';
@session_start();
...
<h4 style="font-size: 14px;  font-style:normal;" class="text-uppercase">Termos e Assinaturas</h4>

listar todos os contratos assinados.(cliente, colaboradore, fornecedore.............). JOIN em todas as tabelas de contratos.
...
<div class="col-md-2"> <!--  Botão com edição para roxo e rosa -->
	<div class="col-md-1">
		 ...
		 <a style="font-size: 12px;  " href="index.php?pagina=<?php echo $pag ?>&funcao=novo" type="button" class="login100-form-btn">Novo Termos</a>
	</div>
</div>
...
<div class="mt-4" style="margin-right:25px">
	<?php 
	$query = $pdo->query("SELECT * from termos_contratos_assinaturas order by id desc");
...
if(@$_GET['funcao'] == "editar"){
	$titulo_modal = 'Editar Termo/Assinatura';
	$query = $pdo->query("SELECT * from termos_contratos_assinaturas where id = '$_GET[id]'");
	$res = $query->fetchAll(PDO::FETCH_ASSOC);
	$total_reg = @count($res);
	if($total_reg > 0){ 
		$valor = $res[0]['valor'];
	...
	$titulo_modal = 'Inserir Termo/Assinatura';
}
?>
		...
		<div class="modal-content">
			...
			<form method="POST" id="form-baixar">
				<div class="modal-body">
					...
					<p>Deseja Realmente confirmar o Recebimento do pagamento desta termos_contratos_assinaturas?</p>
...
<script type="text/javascript">
	...
	function carregarImg() {
        ...
        if(resultado[1] === 'pdf'){
        	$('#target').attr('src', "../img/termos_contratos_assinaturas/pdf.png");
        	return;
        }
