...
<h4 style="font-size: 14px;  font-style:normal;" class="text-uppercase">Gravar Padrão de Atendimento</h4>
...
<div class="col-md-7"> <!--  Botão  -->
	<div class="col-md-4">
		...
		<a style="font-size: 12px;  " href="index.php?pagina=<?php echo $pag ?>&funcao=novo"
		 type="button" class="btn btn-outline-primary">Novo Padrão de Atendimento</a>
	</div>
</div>
...
<div class="mt-4" style="margin-right:25px">
	<?php 
	$query = $pdo->query("SELECT * from padrao_atendimento order by id desc");
	...
	if($total_reg > 0){ 
		...
		<small>
			<table id="example" class="table table-hover my-4" style="width:100%">
				<thead>
					<tr>
						<th>Titulo Padrão de Atendimento</th>
						<th>Descrição:</th>					
						<th>Arquivo</th>
						<th></th>
						<th>Ações</th>
				...
				<tbody>
					...
					for($i=0; $i < $total_reg; $i++){
						...
						elseif ($extensao == '.doc') {
							...
							<td>
								<a href="../img/<?php echo $pag ?>/<?php echo $arquivo_pasta ?>" title="Ver Arquivo" style="text-decoration: none" target="_blank">
...
if(@$_GET['funcao'] == "editar"){
	$titulo_modal = 'Editar Padrão de Atendimento';
	$query = $pdo->query("SELECT * from padrao_atendimento where id = '$_GET[id]'");
	$res = $query->fetchAll(PDO::FETCH_ASSOC);
	$total_reg = @count($res);
	if($total_reg > 0){ 
		$nome = $res[0]['nome'];
	...
	$titulo_modal = 'Inserir Padrão de Atendimento';
}
?>
			...
			<form method="POST" id="form">
				<div class="modal-body">
					...
					<div class="row">
						...
						<div class="col-md-12">
							<div class="mb-5">
								<label for="exampleFormControlInput1" class="form-label">Titulo Padrão de Atendimento</label>
...
<script type="text/javascript">
	function mostrarDados(nome, foto){
		...
		$('#imagem-registro').attr('src', '../img/padrao_atendimento/' + foto);
