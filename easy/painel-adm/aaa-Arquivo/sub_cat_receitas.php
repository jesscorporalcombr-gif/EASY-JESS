...
<h4 style="font-size: 14px;  font-style:normal;" class="text-uppercase">Sub Categoria de Receitas</h4>
...
<div class="col-md-6"> <!--  Botão com edição para roxo e rosa -->
	<div class="col-md-6">
		...
		<a style="font-size: 12px;  " href="index.php?pagina=<?php echo $pag ?>&funcao=novo"
		 type="button" class="btn btn-outline-primary">Nova Sub Categoria de Receita</a>
	</div>
</div>
...
<div class="mt-4" style="margin-right:25px">
	<?php 
	$query = $pdo->query("SELECT * from sub_cat_receitas order by id desc");
	...
	if($total_reg > 0){ 
		...
		<small>
			<table id="example" class="table table-hover my-4" style="width:100%">
				<thead>
					<tr>
						<th>Nome Sub Categoria de Receita</th>
				...
				<tbody>
					...
					for($i=0; $i < $total_reg; $i++){
						foreach ($res[$i] as $key => $value){	}
							...
							$query_2sub_ = $pdo->query("SELECT * from receitas where id = '$id_sub_cat'");
...
if(@$_GET['funcao'] == "editar"){
	$titulo_modal = 'Editar Sub Categoria de Receita';
	$query = $pdo->query("SELECT * from sub_cat_receitas where id = '$_GET[id]'");
	$res = $query->fetchAll(PDO::FETCH_ASSOC);
	$total_reg = @count($res);
	if($total_reg > 0){ 
		$nome = $res[0]['nome'];
		...
		$query_2sub_2 = $pdo->query("SELECT * from receitas where id = '$id_sub_cat2'");
	...
	$titulo_modal = 'Inserir Sub Categoria de Receita';
}
?>
			...
			<form method="POST" id="form">
				<div class="modal-body">
					...
					<div class="row">
						...
						<div class="col-md-12">
							<div class="mb-5">
								<label for="exampleFormControlInput1" class="form-label">Nome da Sub Categoria de Receita</label>
						...
						<div class="col-md-12">
							<div class="mb-5">
								...
								<label for="exampleFormControlInput1" class="form-label">Descrição da Sub Categoria de Receita</label>
					...
					<div class="col-md-6">
						<div class="mb-3"><!-- Arrumar o salvamento em tabela-->
							<label for="exampleFormControlInput1" class="form-label">Categoria Referente</label>


								<!-- Busca do banco de dados para uma  select. Escolha dinamica -->
				              <?php
				                 $sql_code_receitas = "SELECT * FROM receitas ORDER BY nome ASC";
...
<div class="modal fade" tabindex="-1" id="modalDeletar" >
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title">Excluir Sub Categoria de Receita</h5>
				<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
			</div>
...
<script type="text/javascript">
	function mostrarDados(nome, foto){
		...
		$('#imagem-registro').attr('src', '../img/sub_cat_receitas/' + foto);
