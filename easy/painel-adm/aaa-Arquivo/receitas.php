...
<h4 style="font-size: 14px;  font-style:normal;" class="text-uppercase">Categoria de Receitas</h4>
...
<div class="col-md-7"> <!--  Botão com edição para roxo e rosa -->
	<div class="col-md-4">
		...
		<a style="font-size: 12px;  " href="index.php?pagina=<?php echo $pag ?>&funcao=novo"
		 type="button" class="btn btn-outline-primary">Nova Categoria de Receita</a>
	</div>
</div>
	...
	if($total_reg > 0){ 
		...
		<small>
			<table id="example" class="table table-hover my-4" style="width:100%">
				<thead>
					<tr>
						<th>Nome Categoria de Receita</th>
						<th>Descrição</th>
...
if(@$_GET['funcao'] == "editar"){
	$titulo_modal = 'Editar Categoria de Receita';
	$query = $pdo->query("SELECT * from receitas where id = '$_GET[id]'");
	$res = $query->fetchAll(PDO::FETCH_ASSOC);
	$total_reg = @count($res);
	if($total_reg > 0){ 
		$nome = $res[0]['nome'];
	...
	$titulo_modal = 'Inserir Categoria de Receita';
}
?>
			...
			<form method="POST" id="form">
				<div class="modal-body">
					...
					<div class="row">
						...
						<div class="col-md-12">
							<div class="mb-5">
								<label for="exampleFormControlInput1" class="form-label">Nome da Categoria de Receita</label>
						...
						<div class="col-md-12">
							<div class="mb-5">
								...
								<label for="exampleFormControlInput1" class="form-label">Descrição da Categoria de Receita</label>
...
<div class="modal fade" tabindex="-1" id="modalDeletar" >
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title">Excluir Categoria de Receita</h5>
				<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
			</div>
...
<script type="text/javascript">
	function mostrarDados(nome, foto){
		...
		$('#imagem-registro').attr('src', '../img/receitas/' + foto);
