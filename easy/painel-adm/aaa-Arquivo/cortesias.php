...
<h4 style="font-size: 14px;  font-style:normal;" class="text-uppercase">Cortesia</h4>

<div class="col-md-7"> <!--  Botão  -->
	<div class="col-md-4">
		</br>

		 <a style="font-size: 12px;  " href="index.php?pagina=<?php echo $pag ?>&funcao=novo" type="button" class="btn btn-outline-primary">Nova Cortesia</a>
	</div>
</div>
...
<div class="mt-4" style="margin-right:25px">
	<?php 
	$query = $pdo->query("SELECT * from cortesias order by id desc");
...
if(@$_GET['funcao'] == "editar"){
	...
	$query = $pdo->query("SELECT * from cortesias where id = '$_GET[id]'");
	$res = $query->fetchAll(PDO::FETCH_ASSOC);
	$total_reg = @count($res);
	if($total_reg > 0){ 
		$valor = $res[0]['valor'];
		...
		<div class="modal-content">
			...
			<form method="POST" id="form">
				<div class="modal-body">
					...
					<div class="mb-3">
						<label for="exampleFormControlInput1" class="form-label">Descrição Cortesias</label>

						

						<textarea type="text" class="form-control" id="descricao" name="descricao"  placeholder="Descrição da Cortesias" maxlength="200"><?php echo @$descricao ?></textarea>
					</div> 
...
<script type="text/javascript">
	...
	function carregarImg() {
        ...
        if(resultado[1] === 'pdf'){
        	$('#target').attr('src', "../img/Cortesia/pdf.png");
        	return;
        }
