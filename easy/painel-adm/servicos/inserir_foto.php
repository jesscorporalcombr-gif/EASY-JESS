...
try {
  ...
  $idServico  = isset($_POST['id_servico']) ? (int)$_POST['id_servico'] : (int)($_POST['id_cliente'] ?? 0);
  $titulo     = trim($_POST['titulo'] ?? '');
  $dataFoto   = $_POST['data_foto'] ?? null;  // YYYY-MM-DD
  $tipoFoto   = trim($_POST['tipo_foto'] ?? ''); // "Clínico", "Antes e Depois", etc.
  $descricao  = trim($_POST['descricao'] ?? '');

  if ($idServico <= 0) jexit(false, 'Serviço não informado.');
  if ($titulo === '') jexit(false, 'Informe o título.');

  $pasta = $_SESSION['x_url'] ?? '';
  $baseDir = $pasta ? "../../{$pasta}/img/servicos/galeria/" : "../../img/servicos/galeria/";
  ...
  $oldOri = $oldMin = null;
  if ($idFoto > 0) {
    $ck = $pdo->prepare("SELECT arquivo_ori, arquivo_mini FROM servicos_fotos WHERE id=:id AND id_servico=:s");
    $ck->execute([':id'=>$idFoto, ':s'=>$idServico]);
  ...
  if ($idFoto > 0) {
    ...
    $sql .= " WHERE id=:id AND id_servico=:s";
    $up = $pdo->prepare($sql);
    $up->execute($params);
    ...
    // INSERT
    $ins = $pdo->prepare(
      "INSERT INTO servicos_fotos
        (id_servico, titulo, data_foto, tipo_foto, descricao, arquivo_ori, arquivo_mini, cadastrado)
       VALUES
  ...
  $st = $pdo->prepare("SELECT * FROM servicos_fotos WHERE id=:id");
