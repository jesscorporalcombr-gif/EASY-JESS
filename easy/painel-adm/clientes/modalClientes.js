...
function habilitarEditaCadastro() {
  ...
  const btnSalvar = document.getElementById('btn-salvar_cliente');
...
function iniciarDocumentosclientes(){
  ...
  const idCliente   = document.querySelector('#formCadCliente input[name="id"]').value;
...
window.abrirFormDocumento = function(documentoRow) {
  ...
  container.style.display   = 'block';
    ...
    window.excluirDocumento = function(idDocumento) {
        ...
        $.post('clientes/excluir_documento.php', { id: idDocumento }, res => {
            if (res.success) {
                // recarrega tabela de galeria
                const tbl = document.querySelector('#documentos-container-modal .tablesModCliente');
...
fileInput.addEventListener('change', function() {
  ...
  if (descTextarea.value.trim()) formData.append('descricao', descTextarea.value.trim());
    ...
    $.ajax({
      url: 'clientes/inserir_documento.php',
      type: 'POST',
      data: formData,
      processData: false,
      contentType: false,
...
iniciarDocumentosclientes();

function iniciarFotosclientes(){
  ...
  btnNova.addEventListener('click', () => {
    ...
    window.abrirFormFoto = function(fotoRow) {
        ...
        previewImg.src       = fotoRow.arquivo_mini
                                ? `../img/clientes/galeria/mini/${fotoRow.arquivo_mini}`
                                : 'sem-foto.svg';
    ...
    window.excluirFoto = function(idFoto) {
        ...
        $.post('clientes/excluir_foto.php', { id: idFoto }, res => {
            if (res.success) {
                // recarrega tabela de galeria
                const tbl = document.querySelector('#galeria-container-modal .tablesModCliente');
                const f = tbl.getAttribute('data-filtro');
                tbl.setAttribute('data-filtro', '');
                tbl.setAttribute('data-filtro', f);
            } else {
...
function imagemCropper(){
    ...
    document.getElementById("img-foto_cadCliente").addEventListener("click", function() {
        if (confirm("Deseja tirar uma foto com a webcam?")) {
            abrirWebcam();
        } else {
            document.getElementById("input-foto_cadCliente").click();
        }
    });

    // ====== ARQUIVO ======
    document.getElementById("input-foto_cadCliente").addEventListener("change", function(e) {
    ...
    document.getElementById("btn-crop-ok").addEventListener("click", function() {
        ...
        canvas.toBlob(function(blob) {
            ...
            document.getElementById('img-foto_cadCliente').src = url;

            // Prepara novo arquivo para input file
            const newFile = new File([blob], "foto_cliente.jpg", { type: "image/jpeg" });
            const dataTransfer = new DataTransfer();
            dataTransfer.items.add(newFile);
            document.getElementById('input-foto_cadCliente').files = dataTransfer.files;
    ...
    document.getElementById("btn-crop-cancel").addEventListener("click", function() {
        ...
        document.getElementById('input-foto_cadCliente').value = '';
    });
}
...
imagemCropper();
	...
	$("#formCadCliente").submit(function (event) {
