...
<div class="mt-4" style="margin-right:25px">
	<?php 
	$query = $pdo->query("SELECT * from img_usuario order by id desc");
...
if(@$_GET['funcao'] == "editar"){
	...
	$query = $pdo->query("SELECT * from img_usuario where id = '$_GET[id]'");
	$res = $query->fetchAll(PDO::FETCH_ASSOC);
	$total_reg = @count($res);
	if($total_reg > 0){ 
		$valor = $res[0]['valor'];
	...
	$titulo_modal = 'Inserir Imagem de Colaborador';
}
?>
...
<script type="text/javascript">
	...
	function carregarImg() {
        ...
        if(resultado[1] === 'pdf'){
        	$('#target').attr('src', "../img/img_usuario/pdf.png");
        	return;
        }
