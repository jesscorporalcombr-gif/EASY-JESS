...
<div class="row align-items-end mb-3">
  <div class="col-auto">
    <label class="form-group">Modelo de anamnese</label>
    <select id="selForm" class="form-select" style="min-width:320px">
      <option value="">Carregando formulários...</option>
    </select>
  </div>
  <div class="col-auto">
    <label class="form-group">Paciente (opcional p/ link)</label>
    <input id="inpPacienteId" type="text" class="form-control" placeholder="ID do paciente">
  </div>
  <div class="col-auto">
    <label class="form-group">&nbsp;</label><br>
      <button style="height: 25px; padding-top: 1px;" id="btnNovo" class="btn btn-primary">Novo</button>
      <button style="height: 25px; padding-top: 1px;" id="btnGerarLink" class="btn btn-light">Gerar link</button>
  </div>
</div>

<div id="boxLink" style="display:none" class="alert alert-info mb-3"></div>
		...
		<div class="table-container" id="agendamentos-container-modal">
			...
			<div id="gridAnamnese"></div>
			<table 
				class="tablePagAnamnses dataTable dTlinhaFina"
 ...
 <script type="text/javascript" src="tecnico/anamnese/tabelaPagAnamneses.js?v=0.11"> </script>
<!--<script src="forms/form_renderer.js"></script> -->
<script type="text/javascript" src="tecnico/anamnese/anamnese.page.js?v=0.12"> </script>



 <script>
  document.addEventListener('DOMContentLoaded', function(){
    // onAction é opcional; se não quiser ainda, omite o segundo argumento
    TabelaAnamneses.render('gridAnamnese', async (act, row)=>{
      // aqui você pode chamar openFillModal, validar, etc.
      // ex.: if (act==='editar') openFillModal({ response_id: row.anamnese_id });
    });
  });
</script>
