// salas/tabModSalaGaleria.js  (v0.15)
...
(function(){
  ...
  function basePaths(){
    ...
    const base  = pasta ? `../${pasta}/img/salas/galeria/` : `../img/salas/galeria/`;
  ...
  function getIds(){
    const idSala = parseInt(document.querySelector('#formCadSala input[name="id"]')?.value || '0', 10);
    return { idSala };
  }
  ...
  async function fetchRows(q=''){
    const { idSala } = getIds();
    const url = `salas/SModSalaTabGaleria.php?id_sala=${encodeURIComponent(idSala)}&q=${encodeURIComponent(q)}`;
  ...
  function bindEditor(){
    ...
    btnSalvar.addEventListener('click', async ()=>{
      const { idSala } = getIds();
      if (!idSala) return alert('Sala inválida.');
      ...
      try {
        const r = await fetch('salas/inserir_foto.php', { method: 'POST', body: fd, credentials: 'same-origin' });
        const j = await r.json();
  ...
  window.initTabModSalaGaleria = initTabela;
