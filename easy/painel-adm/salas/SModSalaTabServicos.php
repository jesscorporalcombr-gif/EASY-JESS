...
try {
  $id_sala = getParam('id_sala');
  if ($id_sala === null || $id_sala === '' || !is_numeric($id_sala)) {
    out(false, [], 400, ['error' => 'Parâmetro id_sala ausente ou inválido.']);
  }
  $id_sala = (int)$id_sala;
  ...
  $map = [
    'id_sala'        => 'CAST(:id_sala AS SIGNED) AS id_sala',
  ...
  if (empty($requested)) {
    foreach (['id_sala','id_servico','servico', 'categoria', 'foto_servico','categoria','id_link','executa'] as $f) {
      if (isset($map[$f])) $requested[$f] = $map[$f];
    }
  }
  ...
  foreach ($rows as &$r) {
    if (isset($r['id_sala']))     $r['id_sala'] = (int)$r['id_sala'];
  ...
  out(false, [], 500, ['error' => 'Erro ao listar serviços para a sala.', 'detail' => $e->getMessage()]);
}
