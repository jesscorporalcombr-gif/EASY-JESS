...
function iniciarTabelaPagColaboradores (){
    ...
    tableContainers.forEach(container => {
      ...
      const dataTableElement = container.querySelector('.tablePagColaboradores');
      ...
      function buildUrl() {
        ...
        // base da URL
        let url = `colaboradores/searchModColaboradores.php?table=${dataTableElement.getAttribute('data-table')}&fields=${fields}&all=true`;
              ...
              ths.forEach(th => {
                    ...
                    if (th.hasAttribute('data-modal')) {
                          ...
                          td.addEventListener('click', function(e) {
                            abrirModal(th.getAttribute('data-modal'), row['contrato_id'], dataTipo);
                      });
                    }
                    ...
                    if (th.hasAttribute('data-foto')) {
                        ...
                        if (cellValue!==''){
                          basePath = `../${pastaFiles}/img/cadastro_colaboradores/`;
                          imgPath = cellValue;
                        } else{
                        basePath = '../img/';
                      ...
                      td.textContent = 'R$ ' + DecimalBr(cellValue);
                        ...
                        td.setAttribute('data-raw', raw);
                          ...
                          var status = cellValue;              // ex: "Lead", "Ativo"…
                          const span   = document.createElement('span');
                          let cls      = 'status-Prof';         // classe base

                          // define classe extra conforme o status
                          switch (status) {
                            case 1:
                              cls += ' status-profAtivo';
                              status = 'Ativo';
                              break;
                            default:
                              cls += ' status-profInativo';
                              status = 'Encerrado';
                          }
                    ...
                    } else if (field === 'situacao' && cellValue) {
