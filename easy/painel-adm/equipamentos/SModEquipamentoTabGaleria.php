...
try {
  $idEquipamento = isset($_GET['id_equipamento']) ? (int)$_GET['id_equipamento'] : 0;
  $q         = trim($_GET['q'] ?? '');

  if ($idEquipamento <= 0) jexit(false, 'id_equipamento inválido');

  $sql = "SELECT id, id_equipamento, titulo, data_foto, tipo_foto, descricao, arquivo_ori, arquivo_mini
          FROM equipamentos_fotos
          WHERE id_equipamento = :s";

  $params = [':s' => $idEquipamento];
