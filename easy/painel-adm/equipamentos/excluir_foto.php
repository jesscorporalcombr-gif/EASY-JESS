...
try {
  ...
  $st = $pdo->prepare("SELECT id, id_sala, arquivo_ori, arquivo_mini FROM salas_fotos WHERE id = :id");
  $st->execute([':id' => $id]);
  $row = $st->fetch(PDO::FETCH_ASSOC);
  if (!$row) jexit(false, 'Foto não encontrada.');

  // remove do banco
  $del = $pdo->prepare("DELETE FROM salas_fotos WHERE id = :id");
  $del->execute([':id' => $id]);

  // monta paths para remover arquivos
  $pasta  = $_SESSION['x_url'] ?? '';
  $base   = $pasta ? "../../{$pasta}/img/salas/galeria/" : "../../img/salas/galeria/";
  $mini   = $base . "mini/";
