...
try {
  ...
  $idEquipamento  = isset($_POST['id_equipamento']) ? (int)$_POST['id_equipamento'] : '';
  $titulo     = trim($_POST['titulo'] ?? '');
  $dataFoto   = $_POST['data_foto'] ?? null;  // YYYY-MM-DD
  $tipoFoto   = trim($_POST['tipo_foto'] ?? ''); // "Clínico", "Antes e Depois", etc.
  $descricao  = trim($_POST['descricao'] ?? '');

  if ($idEquipamento <= 0) jexit(false, 'Serviço não informado.');
  if ($titulo === '') jexit(false, 'Informe o título.');

  $pasta = $_SESSION['x_url'] ?? '';
  $baseDir = $pasta ? "../../{$pasta}/img/equipamentos/galeria/" : "../../img/equipamentos/galeria/";
  ...
  $oldOri = $oldMin = null;
  if ($idFoto > 0) {
    $ck = $pdo->prepare("SELECT arquivo_ori, arquivo_mini FROM equipamentos_fotos WHERE id=:id AND id_equipamento=:s");
    $ck->execute([':id'=>$idFoto, ':s'=>$idEquipamento]);
  ...
  if ($idFoto > 0) {
    // UPDATE
    $sql = "UPDATE equipamentos_fotos SET
              titulo=:titulo,
              data_foto=:data_foto,
              tipo_foto=:tipo_foto,
              descricao=:descricao";
    ...
    $sql .= " WHERE id=:id AND id_equipamento=:s";
    $up = $pdo->prepare($sql);
    $up->execute($params);
    ...
    // INSERT
    $ins = $pdo->prepare(
      "INSERT INTO equipamentos_fotos
        (id_equipamento, titulo, data_foto, tipo_foto, descricao, arquivo_ori, arquivo_mini, cadastrado)
       VALUES
  ...
  $st = $pdo->prepare("SELECT * FROM equipamentos_fotos WHERE id=:id");
