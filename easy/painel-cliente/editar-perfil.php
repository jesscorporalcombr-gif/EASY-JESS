...
if($antigo2 != $email){
	...
	if(@count($res_con) > 0){
		echo 'O email do clientes já está cadastrado!';
		exit();
	}
}
...
// EVITAR DUPLICIDADE NO CPF
	...
	$res_con = $query_con->fetchAll(PDO::FETCH_ASSOC);
	if(@count($res_con) > 0){
		echo 'O CPF do clientes já está cadastrado!';
		exit();
	}
}


	$res = $pdo->prepare("UPDATE clientes SET nome = :nome, email = :email, cpf = :cpf, senha = :senha WHERE id = :id");
