...
<nav class="navbar navbar-expand-lg navbar-light bg-light">
	 ...
	 <h4 style="font-size: 14px;  font-style:normal;" class="text-uppercase">&nbsp;&nbsp;&nbsp;&nbsp;Agendamentos</h4> 
    ...
    <div class="row"> <!-- inicio grupo  Mostra os totais do cliente -->
		<?php 
			...
			@$id_clientes0 = $nome_usu;
			@$query_clientes0 = $pdo->query("SELECT * from clientes where nome = '$id_clientes0'");
...
<div class="mt-4" style="margin-right:25px">
	<?php 
	$query = $pdo->query("SELECT * from agendar_conectado where cliente = '$nome_clientes0' order by data asc");
	$res = $query->fetchAll(PDO::FETCH_ASSOC);
	$total_reg = @count($res);
	if($total_reg > 0){ 
		?>
				...
				<tbody>
					...
					for($i=0; $i < $total_reg; $i++){
						...
						<tr>
							...
							<td><a href="../img/agendar_conectado/<?php echo $res[$i]['arquivo'] ?>" title="Ver Arquivo" style="text-decoration: none" target="_blank">
								<img src="../img/agendar_conectado/<?php echo $arquivo_pasta ?>" width="40">
							</a>
						</td>
						<td>
			...
			<form method="POST" id="form">
					...
					<div class="col-md-3">
						 <div class="mb-3">				
							...
							<select class="form-select mt-1" aria-label="Default select example" name="fim">
									...
									<option <?php if(@$fim == '0700'){ ?> selected <?php } ?>  value="0700">07:00</option>
									<option <?php if(@$fim == '0730'){ ?> selected <?php } ?>  value="0730">07:30</option>
									<option <?php if(@$fim == '0800'){ ?> selected <?php } ?>  value="0800">08:00</option>
									...
									<option <?php if(@$fim == '1400'){ ?> selected <?php } ?>  value="1400">14:00</option>
									<option <?php if(@$fim == '1430'){ ?> selected <?php } ?>  value="1430">14:30</option>
						 ...
						 <div class="mb-3">			
							<label for="exampleFormControlInput1" class="form-label">Status</label>
								<select class="form-select mt-1" aria-label="Default select example" name="status">
									...
									<option <?php if(@$status == 'Bloquear Horário'){ ?> selected <?php } ?>  value="Bloquear Horário">Bloquear Horário</option>
			    ...
			    <div class="mb-3">
					...
					<div id="divImgConta" class="mt-4">
						<?php if(@$arquivo != ""){ ?>
							<img src="../img/agendar_conectado/<?php echo @$arquivo_pasta2 ?>"  width="200px" id="target">
						<?php  }else{ ?>
							<img src="../img/agendar_conectado/sem-foto.jpg" width="200px" id="target">
						<?php } ?>
					</div>
