...
<h4 style="font-size: 14px;  font-style:normal;" class="text-uppercase">Sugestões</h4>
...
<div class="col-md-7"> <!--  Botão -->
	<div class="col-md-3">
		...
		<a style="font-size: 12px;  " href="index.php?pagina=<?php echo $pag ?>&funcao=novo"
		 type="button" class="btn btn-outline-primary">Nova Sugestão</a>
	</div>
</div></br></br>
...
<div class="mt-4" style="margin-right:25px">
	<?php 
	$query = $pdo->query("SELECT * from sujestao order by id DESC");
	$res = $query->fetchAll(PDO::FETCH_ASSOC);
	$total_reg = @count($res);
	if($total_reg > 0){ 
		?>
					...
					for($i=0; $i < $total_reg; $i++){
						...
						<tr>
							...
							<?php 
								...
								$query2 = $pdo->query("SELECT * from clientes WHERE id = '$id_criador'");
...
if(@$_GET['funcao'] == "editar"){
	...
	$query = $pdo->query("SELECT * from sujestao where id = '$_GET[id]'");
	$res = $query->fetchAll(PDO::FETCH_ASSOC);
	$total_reg = @count($res);
	if($total_reg > 0){ 
		...
		<div class="modal-content">
			...
			<form method="POST" id="form">
				<div class="modal-body">
					...
					<div class="col-md-10">
							<div class="mb-5">
								<label for="exampleFormControlInput1" class="form-label">sujestao</label>
