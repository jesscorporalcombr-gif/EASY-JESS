...
<h4 style="font-size: 14px;  font-style:normal;" class="text-uppercase">Comissões</h4>
...
<div class="col-md-2"> <!--  Botão com edição para roxo e rosa -->
	<div class="col-md-1">
		 ...
		 <a style="font-size: 12px;  " href="index.php?pagina=<?php echo $pag ?>&funcao=novo" type="button" class="login100-form-btn">Nova Comissão</a>
	</div>
</div>
...
<div class="mt-4" style="margin-right:25px">
	<?php 
	$query = $pdo->query("SELECT * from comissoes order by id desc");
...
if(@$_GET['funcao'] == "editar"){
	...
	$query = $pdo->query("SELECT * from comissoes where id = '$_GET[id]'");
	$res = $query->fetchAll(PDO::FETCH_ASSOC);
	$total_reg = @count($res);
	if($total_reg > 0){ 
		$valor = $res[0]['valor'];
		...
		<div class="modal-content">
			...
			<form method="POST" id="form-baixar">
				<div class="modal-body">
					...
					<p>Deseja Realmente confirmar o Recebimento do pagamento desta comissoes?</p>
...
<script type="text/javascript">
	...
	function carregarImg() {
        ...
        if(resultado[1] === 'pdf'){
        	$('#target').attr('src', "../img/comissoes/pdf.png");
        	return;
        }
